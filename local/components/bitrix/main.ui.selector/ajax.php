...
class CMainUISelectorComponentAjaxController extends \Bitrix\Main\Engine\Controller
{
	...
	public function getDataAction(array $options = array(), array $entityTypes = array(), array $selectedItems = array())
	{
	    ...
	    if($options['context'] == 'USER_LIST_FILTER_DEPARTMENT' ||  true){
	        $result =  \Bitrix\Main\UI\Selector\Entities::getData($options, $entityTypes, $selectedItems);
	       
	        if (CModule::IncludeModule('fbit.inspector')){
	            $Inspector = new \Fbit\Inspector\Filter();
	            $Inspector->PrepareStructureDepartment($result['ENTITIES']['DEPARTMENTS']['ITEMS']);
