...
?>
    <div id="<?= htmlspecialcharsbx($toolbarID) ?>"
         class="pagetitle-container pagetitle-align-right-container crm-pagetitle-btn-box"><?

$bindingMenuMask = '/(lead|deal|invoice|quote|company|contact).*?([\d]+)/i';
if (
    preg_match($bindingMenuMask, $arParams['TOOLBAR_ID'], $bindingMenuMatches) &&
...
if ($communicationPanel) {
    $data = isset($communicationPanel['DATA']) && is_array(
        $communicationPanel['DATA']
    ) ? $communicationPanel['DATA'] : array();
    ...
    <div class="crm-entity-actions-container">
        <? if (!$enableCall || empty($phones)) {
            ?>
        <div id="<?= htmlspecialcharsbx($callButtonId) ?>"
             class="ui-btn ui-btn-light-border ui-btn-icon-phone-call ui-btn-disabled ui-btn-themes"></div><?
        } else {
            ?>
        <div id="<?= htmlspecialcharsbx($callButtonId) ?>"
             class="ui-btn ui-btn-light-border ui-btn-icon-phone-call ui-btn-themes"></div><?
        } ?>
        ...
        <? if (empty($emails)) {
            ?>
        <div id="<?= htmlspecialcharsbx($emailButtonId) ?>"
             class="ui-btn ui-btn-light-border ui-btn-icon-mail ui-btn-disabled ui-btn-themes"></div><?
        } else {
            ?>
        <div id="<?= htmlspecialcharsbx($emailButtonId) ?>"
             class="ui-btn ui-btn-light-border ui-btn-icon-mail ui-btn-themes"></div><?
        } ?>
        <script type="text/javascript">
            BX.ready(
                function () {
        ...
        <? if (empty($messengers)) {
            ?>
        <div id="<?= htmlspecialcharsbx($messengerButtonId) ?>"
             class="ui-btn ui-btn-light-border ui-btn-icon-chat ui-btn-disabled ui-btn-themes"></div><?
        } else {
            ?>
        <div id="<?= htmlspecialcharsbx($messengerButtonId) ?>"
             class="ui-btn ui-btn-light-border ui-btn-icon-chat ui-btn-themes"></div><?
        } ?>
        <script type="text/javascript">
            BX.ready(
                function () {
...
if ($documentButton) {
    ...
    ?>
    <button class="ui-btn ui-btn-md ui-btn-light-border ui-btn-dropdown ui-btn-themes crm-btn-dropdown-document"
            id="<?= htmlspecialcharsbx($documentButtonId); ?>"><?= $documentButton['TEXT']; ?></button>
    <script>
        BX.ready(function () {
            if (BX.DocumentGenerator && BX.DocumentGenerator.Button) {
                var button = new BX.DocumentGenerator.Button('<?=htmlspecialcharsbx(
                    $documentButtonId
                );?>', <?=CUtil::PhpToJSObject($documentButton['PARAMS']);?>);
                button.init();
            } else {
...
foreach ($items as $item) {
    ...
    ?>
        <div class="ui-btn-split ui-btn-primary">
            <button id="<?= htmlspecialcharsbx($labelID); ?>" class="ui-btn-main"><?= htmlspecialcharsbx(
                    $schemeDescr
                ) ?></button>
    ...
    ?>
        <button class="ui-btn ui-btn-md ui-btn-light-border ui-btn-themes crm-bizproc-starter-icon"
                id="<?= htmlspecialcharsbx($starterButtonId) ?>"
                title="<?= GetMessage('CRM_TOOLBAR_BIZPROC_STARTER_LABEL') ?>"></button>
        <script type="text/javascript">
            BX.ready(
                function () {
    ...
    <?
        ...
        ?><a target="_top"
             class="webform-small-button webform-small-button-blue crm-top-toolbar-add<?= $icon !== '' ? " {$icon}" : '' ?>"
             href="<?= $link ?>" title="<?= $title ?>"<?= $onClick !== '' ? " onclick=\"{$onClick}; return false;\"" : '' ?>><?= $text ?></a><?
    }
}
?></div><?
$this->EndViewTarget();
