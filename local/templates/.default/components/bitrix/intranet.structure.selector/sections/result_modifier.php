...
// Доработка модуля "Наблюдатели"
$Filter = array("IBLOCK_ID" => $iblockID, "SECTION_ID" => $current_section, 'GLOBAL_ACTIVE' => 'Y');
if (CModule::IncludeModule('fbit.inspector') && $mode != 'subtree'){
    $Inspector = new \Fbit\Inspector\Filter();
    $Inspector->GetFilter($Filter);
   
    $current_section = $Inspector->CheckCurrent($current_section);
}
