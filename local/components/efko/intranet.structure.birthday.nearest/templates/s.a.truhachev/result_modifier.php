...
if ($IBLOCK_ID !== false) {
    ...
    $arComparison = [];
    while ($arRes = $dbRes->Fetch()) {
        array_push($arComparison, $arRes['NAME']);
    }
        $duplicate = false;
    foreach(array_count_values($arComparison) as $item)
    {
        if($item> 1)
        {
            $duplicate = true;
    ...
    if($duplicate == true)
    {
        $arComparison = array_unique(array_diff_assoc($arComparison,array_unique($arComparison)));
    }
}
if ($IBLOCK_ID !== false)
{
    ...
    while ($arRes = $dbRes->Fetch())
    {
        if(in_array($arRes['NAME'],$arComparison))
        {
            $departmentTop = \Bitrix\Main\UserUtils::getDepartmentName($arRes['ID']);
            $result = \Bitrix\Main\UserUtils::getDepartmentName($departmentTop['IBLOCK_SECTION_ID']);
            $arSections[$arRes['ID']] = trim(str_repeat('-', $arRes['DEPTH_LEVEL']-1).' '.$arRes['NAME'].' ('.$result["NAME"].')');
        }
        else{
...
foreach($arParams["DEPARTMENT_REMUVE"] as $value){
    if(isset($arSections[$value]))
        unset($arSections[$value]);
}
$arParams["DEPARTMENT_REMUVE"] = $arSections;
