...
if(CModule::IncludeModule('fbit.inspector')){
    $Inspector = new \Fbit\Inspector\Filter();
    $Inspector->GetDeleteDepatments();
    $deleteDepart = $Inspector->DeleteDepatments;
    $Section = \Bitrix\Iblock\SectionTable::GetList(['filter' => ['!=ID' => $deleteDepart, 'IBLOCK_ID' => $arResult['UF_DEPARTMENT_field']['SETTINGS']['IBLOCK_ID']], 'select' => ['ID', 'NAME'], 'order' => ['LEFT_MARGIN' => 'ASC']])->FetchAll();
    
    $arResult['departmens'] = [
        'REFERENCE' => array_column($Section, 'NAME'),
        'REFERENCE_ID' => array_column($Section, 'ID')
    ];
